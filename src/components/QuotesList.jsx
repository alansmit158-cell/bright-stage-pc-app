...
const QuotesList = () => {
    ...
    const handleDownload = async (id, eventName, companyId = 'bright') => {
        try {
            const response = await api.get(`/projects/${id}/pdf?type=quote&companyId=${companyId}`, { responseType: 'blob' });
                    ...
                    <tbody>
                        {quotes.map((q, index) => (
                            <tr key={q._id} style={{
                                ...
                                <td style={{ padding: '20px 24px', color: '#e2e8f0' }}>{q.clientId?.name || q.client?.name || '-'}</td>
                ...
                <div className="modal-overlay-blur" style={{ 
                    ...
                    <div className="premium-modal" style={{ 
                        ...
                        <div style={{ marginBottom: '25px' }}>
                            ...
                            <p style={{ color: '#94a3b8', marginTop: '10px' }}>Choose the company profile for this quote PDF</p>
                        </div>

                        <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                            <button 
                                onClick={() => {
                                    handleDownload(activeQuote._id, activeQuote.eventName, 'bright');
                            ...
                            <button 
                                onClick={() => {
                                    handleDownload(activeQuote._id, activeQuote.eventName, 'square');
                                    setShowCompanySelect(false);
                                }}
                                style={{ 
                                    padding: '16px', 
