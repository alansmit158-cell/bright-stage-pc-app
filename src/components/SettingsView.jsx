...
const SettingsView = () => {
    ...
    const handleSaveCompany = async () => {
        try {
            ...
            showSuccess(`${selectedCompany === 'bright' ? 'Bright Stage' : 'Square Event'} configuration synchronized!`, "Settings Updated");
        } catch (err) {
            showError('System sync failure: ' + err.message);
        }
    };
    ...
    const handleSeedCurrencies = async () => {
        const ok = await confirm({
            title: 'Matrix Currency Initialization',
            message: 'Seed default currencies (DT, EUR, USD)? This will establish baseline exchange rates and symbols in your system.',
        ...
        try {
            ...
            showSuccess('Fiscal currencies initialized successfully', "Initialization Complete");
            fetchData();
        } catch (err) {
            if (err.response?.status === 403) {
                showError("Access Denied: You do not have permission to seed system data", "Permission Error");
            } else {
                showError('Currency initialization failed: ' + (err.response?.data?.error || err.message));
            }
        }
    };
    ...
    const handleSeedAccounts = async () => {
        const ok = await confirm({
            ...
            message: 'Seed standard Chart of Accounts for the operational company?',
        ...
        try {
            ...
            showSuccess('Chart of Accounts generated for ' + selectedCompany, "Accounting Sync");
            fetchData();
        } catch (err) {
            showError('Account seeding failed: ' + (err.response?.data?.error || err.message));
        }
    };
