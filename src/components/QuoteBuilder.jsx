...
const QuoteBuilder = () => {
    ...
    const [inventory, setInventory] = useState([]); // [FIXED] Added missing state
    ...
    const fetchResources = async () => {
        try {
            ...
            console.warn("Professional Services: Some non-critical system resources could not be synchronized.", err);
            setLoading(false);
            // Non-critical: we still allow limited functionality if some lists are empty
        }
    };
    ...
    const handleDateChange = (field, value) => {
        let newDates = { ...formData.dates, [field]: value };

        // Validation: End date cannot be before Start date
        if (field === 'start' && newDates.end && value > newDates.end) {
            newDates.end = value;
        } else if (field === 'end' && newDates.start && value < newDates.start) {
    ...
    const calculateFinancials = () => {
        ...
        setFinancials({ totalExclTax, totalTax, stampDuty, totalCost, margin, marginPercent, totalInclTax });
    };
                        ...
                        <div className="form-group">
                            ...
                            {selectedClient && (
                                <div className="client-info-card">
                                    ...
                                    <p><strong>Total Spent:</strong> {selectedClient.totalSpent?.toFixed(3) || "0.000"} DT</p>
                                </div>
                            )}
                        </div>
                        <div className="form-group">
                    ...
                    <div className="step-panel fade-in">
                                ...
                                <tbody>
                                    {formData.items.map((item, idx) => (
                                        <tr key={item.id}>
                                            ...
                                            <td>
                                                <div className="row-total-cell">
                                                    {((item.price * item.quantity * (item.type === 'Rent' ? item.days : 1)) * (1 - item.discount / 100)).toFixed(3)} DT
                                                </div>
                                            </td>
                                            <td>
                        ...
                        <div className="financials-bar">
                            ...
                            <div className="fin-metric positive">
                                <span>Total (TTC)</span>
                                <strong>{financials.totalInclTax?.toFixed(3) || (financials.totalExclTax + financials.totalTax + 1).toFixed(3)} DT</strong>
                            </div>
                        </div>
                    ...
                    <div className="step-panel fade-in">
                        ...
                        <div className="review-card">
                            ...
                            <div className="total-row large">
                                <span>Total (TTC)</span>
                                <span>{financials.totalInclTax.toFixed(3)} DT</span>
                            </div>
                        </div>
